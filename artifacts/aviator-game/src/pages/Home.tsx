...
const SECTIONS = [
  {
    ...
    label: 'Casino Games',
    count: '52+',
    desc: 'Slots, Roulette, Blackjack, Crash games & Live casino',
    color: '#ff5f1f',
    glow: 'rgba(255,95,31,0.3)',
  },
  {
    href: '/virtual',
    emoji: '🎮',
    label: 'Virtual Games',
    count: '50+',
    desc: 'Virtual Football, Horse Racing, F1, Basketball & more',
    color: '#06b6d4',
  ...
  },
  {
    ...
    count: '51+',
    desc: 'Premier League, Champions League, NBA, UFC & more',
    color: '#22c55e',
  ...
  },
  {
    ...
    label: 'Spin & Win',
    count: '∞',
    desc: 'Spin the wheel for prizes, free plays & bonuses',
    color: '#a855f7',
  ...
  },
  {
    ...
    count: '100+',
    desc: 'Fully-funded scholarships for Liberian students worldwide',
    color: '#a855f7',
    glow: 'rgba(168,85,247,0.3)',
  },
  {
    href: '/visas',
    emoji: '✈️',
    label: 'Free Visa Countries',
    count: '100+',
    desc: 'Countries Liberians can visit visa-free or on arrival',
    color: '#22c55e',
    glow: 'rgba(34,197,94,0.3)',
  },
];
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: 'hsl(220 20% 7%)' }}>
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse 100% 60% at 50% 0%, rgba(255,95,31,0.15) 0%, transparent 70%)',
        }}
      >
        {/* Background emojis */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-10 text-6xl">
          {['✈️', '🎰', '⚽', '🎡', '🎓', '🃏', '🏆', '💎'].map((e, i) => (
            <span key={i} className="absolute"
              style={{ left: `${10 + i * 12}%`, top: `${Math.random() * 80}%` }}>
              {e}
            </span>
          ))}
        </div>
        ...
        <div className="relative z-10 max-w-2xl mx-auto flex flex-col items-center gap-4">
          ...
          <h1 className="font-black text-4xl sm:text-5xl leading-tight">
            Play, Win &{' '}
            <span style={{ color: '#ff5f1f' }}>Go Global</span>
          </h1>
            ...
            <Link href="/play">
              <a
                ...
                style={{
                  ...
                  color: 'white',
                  boxShadow: '0 0 24px rgba(255,95,31,0.5)',
                }}
              >
                ✈️ Play Aviator Now
              </a>
      ...
      <div className="max-w-7xl mx-auto px-4 flex flex-col gap-10">
            ...
            {SECTIONS.map(s => (
              <Link key={s.href} href={s.href}>
                <a
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${s.glow.replace('0.3', '0.1')} 0%, rgba(255,255,255,0.03) 100%)`,
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/casino"><a className="text-xs text-muted-foreground hover:text-foreground no-underline">See all →</a></Link>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {FEATURED.map(game => (
              <Link key={game.id} href={game.id === 'aviator' ? '/play' : '/casino'}>
        ...
        <section className="pb-4">
          <div
            className="p-5 rounded-2xl flex flex-col sm:flex-row items-center gap-4"
            style={{
