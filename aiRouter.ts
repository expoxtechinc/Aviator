...
type ProviderResult = { text: string; provider: ProviderName; model: string };
...
const providers: Array<{ name: ProviderName; call: (messages: OpenAIMessage[]) => Promise<ProviderResult> }> = [
...
export const aiRouter = router({
  chat: protectedProcedure.input(chatInputSchema).mutation(async ({ input }) => {
    ...
    for (const provider of providers) {
      try {
        ...
        return { ...result, fallbackUsed: provider.name !== providers[0].name };
      } catch (error) {
        failures.push(`${provider.name}:${error instanceof Error ? error.message : "failed"}`);
        console.warn(`[AI router] ${provider.name} unavailable`);
      }
    }
  ...
  health: publicProcedure.query(() => ({
    ...
    note: "Provider availability can change because of quotas, outages, or rate limits; requests fail over without exposing keys.",
  })),
});
