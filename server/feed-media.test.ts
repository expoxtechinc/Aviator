...
const styles = readFileSync(resolve(process.cwd(), "client/src/index.css"), "utf8");
 ...
 describe("public Feed media and discovery contracts", () => {
  it("renders public image lightbox and inline audio/video states with load-error feedback", () => {
    ...
    expect(community).toContain("<video controls");
    expect(community).toContain("<audio controls");
    expect(community).toContain("This public media could not be loaded.");
  ...
  it("renders typed published content references without selecting protected originals", () => {
    expect(community).toContain('select("id,title,content_type,price,currency,access_mode,description,slug")');
